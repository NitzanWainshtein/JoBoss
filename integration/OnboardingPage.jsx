          ...
          >
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
              <div>
                ...
                <div style={{ fontSize: 13, color: '#9A8FD0', marginTop: 2 }}>{p.price}</div>
                {p.trial && <div style={{ fontSize: 11, color: '#F5A623', marginTop: 2 }}>✨ {p.trial}</div>}
              </div>
              <div style={{
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginTop: 10 }}>
              {p.features.map(f => (
                <span key={f} style={{ fontSize: 12, background: '#F5F3FC', padding: '3px 10px', borderRadius: 20, color: '#8B82B8' }}>{f}</span>
              ))}
            </div>
          </div>
        ))}
    ...
    <div>
      ...
      {uploadDone && existingResume ? (
        <div style={{ background: '#EBFBF2', border: '1px solid #12A96F', borderRadius: 16, padding: 16, marginTop: 16 }}>
          <div style={{ fontWeight: 700, color: '#0E8A5F', fontSize: 15 }}>✅ {existingResume.fileName || 'קורות חיים קיימים'}</div>
          <div style={{ fontSize: 13, color: '#4A4470', marginTop: 4 }}>ניתן לשנות בפרופיל בכל עת.</div>
          {analyzing && (
          ...
          <label style={{
            ...
            {uploading ? (
              <>
                ...
                <div style={{ fontWeight: 600, color: '#7C5CFF' }}>מעלה...</div>
              </>
            ) : (
              <>
                <div style={{ fontSize: 56 }}>📎</div>
                <div style={{ fontWeight: 700, fontSize: 16, color: '#7C5CFF' }}>לחץ להעלאת קובץ PDF</div>
                <div style={{ fontSize: 13, color: '#C4BCE4' }}>עד 5MB</div>
              </>
            )}
    ...
    <div>
      ...
      {aiSuggestions.length > 0 && (
        <div style={{ margin: '12px 0', background: '#F1ECFF', borderRadius: 16, padding: 14 }}>
          <div style={{ fontSize: 13, fontWeight: 700, color: '#7C5CFF', marginBottom: 10 }}>💡 ה-AI ממליץ לך:</div>
...
function SettingsStep({ location, setLocation, onCoords, radius, setRadius, expLevel, setExpLevel, availability, setAvailability, phone, setPhone, currentLocation, setCurrentLocation, currentCompany, setCurrentCompany, gender, setGender, showAllJobs, setShowAllJobs }) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
      ...
      <div>
        <div style={T.label}>🏢 חברה נוכחית <span style={{ fontWeight: 400, color: '#C4BCE4', fontSize: 13 }}>(אופציונלי)</span></div>
        <input
          type="text"
      ...
      <div>
        <div style={T.label}>⚧ מגדר <span style={{ fontWeight: 400, color: '#C4BCE4', fontSize: 13 }}>(אופציונלי)</span></div>
        <select
          value={gender}
      ...
      <div>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between' }}>
          <span style={{ fontSize: 11, color: '#D5CEE8' }}>5 ק"מ</span>
          <span style={{ fontSize: 11, color: '#D5CEE8' }}>100 ק"מ</span>
        </div>
      </div>

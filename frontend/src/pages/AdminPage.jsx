...
function AdminPage() {
  ...
  const toggleJob = (jobId) => setJobs(prev => prev.map(j => j.jobId === jobId ? { ...j, active: !j.active } : j));
  const deleteJob = (jobId) => setJobs(prev => prev.filter(j => j.jobId !== jobId));
  const addJob = () => {
    ...
    <div style={styles.container}>
      <div style={styles.content}>
        ...
        <div style={styles.statsGrid}>
          ...
          <div style={styles.statCard}>
            <p style={{ ...styles.statNumber, color: '#4CC9F0' }}>{jobs.length}</p>
            <p style={styles.statLabel}>משרות</p>
          </div>
          <div style={styles.statCard}>
            <p style={{ ...styles.statNumber, color: '#1E2A4A' }}>
        ...
        {tab === 'users' && (
          <div style={styles.list}>
            {mockUsers.map((user) => (
              <motion.div key={user.id} style={styles.card} whileHover={{ scale: 1.01 }}>
                ...
                <span style={{
                  ...styles.planBadge,
                  background: user.plan === 'premium' ? '#6C4FD4' : '#eee',
        ...
        {tab === 'jobs' && (
          <div style={styles.list}>
            <button style={styles.addBtn} onClick={() => setShowAddJob(!showAddJob)}>+ הוסף משרה</button>

            {showAddJob && (
              <motion.div style={styles.addJobForm} initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}>
                <input style={styles.input} placeholder="חברה" value={newJob.company} onChange={e => setNewJob({ ...newJob, company: e.target.value })} />
                <input style={styles.input} placeholder="תפקיד" value={newJob.title} onChange={e => setNewJob({ ...newJob, title: e.target.value })} />
                <input style={styles.input} placeholder="מיקום" value={newJob.location} onChange={e => setNewJob({ ...newJob, location: e.target.value })} />
                <input style={styles.input} placeholder="שכר" value={newJob.salary} onChange={e => setNewJob({ ...newJob, salary: e.target.value })} />
                <button style={styles.saveBtn} onClick={addJob}>שמור</button>
              </motion.div>
            )}

            {jobs.map((job) => (
              <motion.div key={job.jobId} style={{ ...styles.card, opacity: job.active ? 1 : 0.5 }} whileHover={{ scale: 1.01 }}>
                <div style={styles.cardInfo}>
                  <p style={styles.cardTitle}>{job.title}</p>
                  <p style={styles.cardSubtitle}>{job.company} · {job.location}</p>
                </div>
                <div style={styles.jobActions}>
                  <button style={{ ...styles.actionBtn, background: job.active ? '#FFC107' : '#4CAF50', color: 'white' }}
                    onClick={() => toggleJob(job.jobId)}>
                    {job.active ? 'השבת' : 'הפעל'}
                  </button>
                  <button style={{ ...styles.actionBtn, background: '#F44336', color: 'white' }}
...
const styles = {
  ...
  adminBadge: { background: 'linear-gradient(135deg, #6C4FD4, #1E2A4A)', color: 'white', padding: '4px 12px', borderRadius: '20px', fontSize: '13px', fontWeight: 700 },
  statsGrid: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(120px, 1fr))', gap: '12px' },
  statCard: { background: 'white', borderRadius: '16px', padding: '20px', textAlign: 'center', boxShadow: '0 2px 8px rgba(108,79,212,0.08)' },
  statNumber: { fontSize: '32px', fontWeight: 800, color: '#6C4FD4', margin: 0 },
  ...
  card: { background: 'white', borderRadius: '16px', padding: '16px 20px', display: 'flex', alignItems: 'center', gap: '16px', boxShadow: '0 2px 8px rgba(108,79,212,0.08)' },
  userAvatar: { width: '44px', height: '44px', borderRadius: '50%', background: 'linear-gradient(135deg, #6C4FD4, #1E2A4A)', display: 'flex', justifyContent: 'center', alignItems: 'center', fontSize: '18px', fontWeight: 800, color: 'white', flexShrink: 0 },
  ...
  addBtn: { background: 'linear-gradient(135deg, #6C4FD4, #1E2A4A)', color: 'white', border: 'none', borderRadius: '12px', padding: '12px', cursor: 'pointer', fontWeight: 700, fontSize: '15px' },
  addJobForm: { background: 'white', borderRadius: '16px', padding: '20px', display: 'flex', flexDirection: 'column', gap: '12px', boxShadow: '0 2px 8px rgba(108,79,212,0.08)' },
